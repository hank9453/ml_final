...
class NewsClassifier(nn.Module):
    def __init__(self, vocab_size, embed_dim, lstm_dim, num_classes):
        ...
        # Batch Normalization
        self.bn = nn.BatchNorm1d(lstm_dim * 2)  # Account for bi-directional output
    ...
    def forward(self, x):
        ...
        # Pass through LSTM
        lstm_out, (hidden, cell) = self.lstm(embedded)  # Shape: (batch_size, seq_length, lstm_dim*2)
        
        # Concatenate forward and backward hidden states (last layer of LSTM)
        final_hidden_state = torch.cat((hidden[-2], hidden[-1]), dim=1)  # Shape: (batch_size, lstm_dim*2)
        
        # Apply BatchNorm and fully connected layer
        normalized = self.bn(final_hidden_state)  # Shape: (batch_size, lstm_dim*2)
...
class NewsClassificationPipeline:
    def __init__(self, data_path, num_words=5000, embed_dim=64, lstm_dim=64, batch_size=32, num_epochs=10 ,feature=['headline']):
...
if __name__ == "__main__":
    pipeline = NewsClassificationPipeline(data_path='Origin.json')
    pipeline.run() 
